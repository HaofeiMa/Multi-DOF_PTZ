...
void videoShow(const sensor_msgs::ImageConstPtr& msg);
...
*/
    ...
    image_transport::Subscriber depth_image_sub;  //定义ROS图像接收器

    //image_sub = it.subscribe("/usb_cam/image_raw", 30, convert_callback);   //定义图像接收器
    image_sub = it.subscribe("/camera/color/image_raw", 30, convert_callback);   //定义图像接收器
    image_pub = it.advertise("image_convert/output_video", 30);  //定义图像发布器
    depth_image_sub = it.subscribe("/camera/extrinsics/depth_to_color", 30, videoShow);   //定义图像接收器
    //cv::namedWindow("Image window");    //初始化输入输出窗口
    cv::namedWindow("机电184马浩飞-基于双目视觉的多自由度云台系统设计-上位机目标跟踪程序");
    cv::setMouseCallback("机电184马浩飞-基于双目视觉的多自由度云台系统设计-上位机目标跟踪程序", on_MouseHandle, (void*)&image);  // 设置鼠标操作回调函数
    serialInit();   // 串口初始化

    sensor_msgs::ImagePtr outputvideo;
    ...
    cv::destroyWindow("机电184马浩飞-基于双目视觉的多自由度云台系统设计-上位机目标跟踪程序");
    return 0;
}
...
void image_process(cv::Mat& image){
    ...
    if (show_visualization) {
        ...
        // 输出图像显示结果
        cv::imshow("机电184马浩飞-基于双目视觉的多自由度云台系统设计-上位机目标跟踪程序", image);
